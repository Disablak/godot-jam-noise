...
func cover(duration = default_cover_duration) -> void:
	...
	_current_tween.tween_property(self, "modulate:a", 1.0, duration).from(0.0).set_trans(cover_transition).set_ease(cover_ease)


func uncover(duration = default_uncover_duration) -> void:
	_current_tween = create_tween()
	_current_tween.tween_property(self, "modulate:a", 0.0, duration).from(1.0).set_trans(uncover_transition).set_ease(uncover_ease)
